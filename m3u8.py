...
class M3u8:
    ...
    def parse(self, url):
        ...
        for ts in response:
            ...
            if '#EXT-X-KEY:' in ts:
                self.encrypt = True
                self.setEncryptKey(ts[ts.index('http'):ts.index('key')+3])
        return container

    def setEncryptKey(self, url):
        '''
        Access to the secret key
        :param url: string, Access to the secret key by the url
        :return: string 需要根据需要修改ts.key 关键字
        '''
        
        self.key = self.request(url=url).content

    def aesDecode(self, data, key):
        '''
        Decode the data 全部是使用byte 字节
    ...
    def download(self, queue, sort, file, downPath, url):
        ...
        if not os.path.exists(debrisName):
            ...
            with open(debrisName, "wb") as f:
                if self.encrypt:
                    data = self.aesDecode(response.content, self.key)
                    f.write(data)
                    f.flush()
                else:
